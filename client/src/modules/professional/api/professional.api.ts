...
interface CancelDayAppointmentsInput {
	date: string;
	reason?: string;
}

interface CancelDayAppointmentsResponse {
	success: boolean;
...
export async function deleteProfessionalOffice(officeId: string) {
	try {
		await apiClient.delete<ApiMessageResponse>(`/offices/${officeId}`);
	} catch (error) {
		throw new Error(getApiErrorMessage(error, "No se pudo eliminar el consultorio."));
	}
}
...
export async function cancelDayAppointments(officeId: string, input: CancelDayAppointmentsInput) {
	try {
		const response = await apiClient.post<CancelDayAppointmentsResponse>(
			`/offices/${officeId}/cancel-day`,
			input,
		);
		return response.data;
	} catch (error) {
		throw new Error(getApiErrorMessage(error, "No se pudieron cancelar los turnos del dia."));
	}
}
