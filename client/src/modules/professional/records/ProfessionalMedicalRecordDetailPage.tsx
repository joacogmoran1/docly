...
function getPatientName(record: Awaited<ReturnType<typeof getMedicalRecord>>) {
  return record.patient?.user
    ? [record.patient.user.name, record.patient.user.lastName].filter(Boolean).join(" ")
    : "Paciente";
}

export function ProfessionalMedicalRecordDetailPage() {
  ...
  const query = useQuery({
    queryKey: [...queryKeys.professionalMedicalRecords, "detail", recordId],
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: async () => {
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.professionalMedicalRecords }),
        queryClient.invalidateQueries({ queryKey: queryKeys.patientMedicalRecords }),
  ...
  return (
    <>
      <SubpageShell
        onBack={() => navigate("/professional/records")}
        headerAction={
          isEditing ? null : (
            <div className="row-wrap">
              <Link to={`/professional/patients/${query.data.patientId}?tab=records`}>
                <Button variant="ghost">Ver ficha</Button>
              </Link>
              <Button onClick={() => setIsEditing(true)}>Editar registro</Button>
      ...
      >
        <Card title="Paciente" className="panel-separated">
          <div className="plain-list">
            <div className="list-row">
              <strong>{getPatientName(query.data)}</strong>
              <span className="meta">{query.data.patient?.user?.email ?? "Sin email"}</span>
            </div>
          </div>
        </Card>
        ...
        {isEditing ? (
          <Card
            ...
            description="Actualiza la informacion clinica del registro."
