...
function applyBlocksToAgenda(agenda: ReturnType<typeof buildAgendaFromSchedules>, blocks: ApiOfficeBlock[]) {
	return agenda.map((entry) => {
		...
		if (!dayBlocks.length) return entry;
		...
		return {
			...
			freeSlots: entry.freeSlots.filter(
				(slot) =>
					!blockedRanges.some((range) => slot >= range.startTime && slot < range.endTime),
			),
			blockedSlots,
...
function mapSchedulesToForm(schedules: ApiSchedule[] | undefined): OfficeRuleDayForm[] {
	...
	activeSchedules.forEach((schedule, index) => {
		const current =
			registry.get(schedule.dayOfWeek) ?? { dayOfWeek: schedule.dayOfWeek, slots: [] };
		current.slots.push({
...
export function ProfessionalOfficeDetailPage() {
	...
	useEffect(() => {
		if (officeQuery.data) setOfficeForm(mapOfficeToForm(officeQuery.data));
	}, [officeQuery.data]);
	...
	const agenda = useMemo(() => {
		...
		const baseAgenda = buildAgendaFromSchedules(
			[officeQuery.data],
			appointmentsQuery.data.filter((a) => a.officeId === officeQuery.data.id),
	...
	const invalidateAll = async () => {
		await Promise.all([
			queryClient.invalidateQueries({ queryKey: [...queryKeys.professionalAppointments, officeId] }),
			queryClient.invalidateQueries({ queryKey: [...queryKeys.professionalOfficeDetail(officeId)] }),
		]);
	};
	...
	const cancelAppointmentMutation = useMutation({
		...
		onSuccess: async () => {
			await invalidateAll();
			setCalendarFeedback({ tone: "success", message: "Turno cancelado correctamente." });
			setAppointmentToCancel(null);
		},
		onError: (error) => {
			setCalendarFeedback({ tone: "error", message: error instanceof Error ? error.message : "No se pudo cancelar el turno." });
			setAppointmentToCancel(null);
		},
	});
	...
	const completeAppointmentMutation = useMutation({
		...
		onSuccess: async () => {
			await invalidateAll();
			setCalendarFeedback({ tone: "success", message: "Turno completado." });
			setAppointmentToComplete(null);
		},
		onError: (error) => {
			setCalendarFeedback({ tone: "error", message: error instanceof Error ? error.message : "No se pudo completar el turno." });
			setAppointmentToComplete(null);
		},
	});
	...
	const blockSlotMutation = useMutation({
		mutationFn: () => {
			if (!slotToBlock) throw new Error("No se pudo preparar el bloqueo.");
			...
			return blockOfficeTimeSlots(officeId, {
				date: selectedDate,
				slots: [{ startTime: slotToBlock.time, endTime: addMinutes(slotToBlock.time, duration) }],
		...
		onSuccess: async () => {
			...
			setCalendarFeedback({
				tone: "success",
				message: slotToBlock ? `Horario bloqueado a las ${slotToBlock.time}.` : "Horario bloqueado.",
			});
			setSlotToBlock(null);
			setBlockReason("");
		},
		onError: (error) => {
			setCalendarFeedback({ tone: "error", message: error instanceof Error ? error.message : "No se pudo bloquear el horario." });
		},
	});
	...
	const blockDayMutation = useMutation({
			...
			setCalendarFeedback({
				...
				message:
					result.cancelledAppointments && result.cancelledAppointments > 0
						? `Dia bloqueado. Se cancelaron ${result.cancelledAppointments} turno(s).`
		...
		onError: (error) => {
			setCalendarFeedback({ tone: "error", message: error instanceof Error ? error.message : "No se pudo bloquear el dia." });
		},
	});

	const cancelDayMutation = useMutation({
		mutationFn: () =>
			cancelDayAppointments(officeId, {
				date: selectedDate,
				reason: cancelDayReason.trim() || "Cancelacion del dia por el profesional",
			}),
		onSuccess: async (result) => {
			await invalidateAll();
			setCalendarFeedback({
				...
				message:
					result.cancelledAppointments > 0
						? `Se cancelaron ${result.cancelledAppointments} turno(s) del ${formatNumericDate(selectedDate)} sin bloquear el dia.`
						: `No habia turnos activos para cancelar el ${formatNumericDate(selectedDate)}.`,
			});
			setIsCancellingDay(false);
			setCancelDayReason("");
		},
		onError: (error) => {
			setCalendarFeedback({ tone: "error", message: error instanceof Error ? error.message : "No se pudieron cancelar los turnos." });
		},
	});
	...
	const unblockBlockMutation = useMutation({
		...
		onError: (error) => {
			setCalendarFeedback({ tone: "error", message: error instanceof Error ? error.message : "No se pudo desbloquear." });
			setBlockToUnblock(null);
		},
	});

	const deleteOfficeMutation = useMutation({
		mutationFn: () => deleteProfessionalOffice(officeId),
		onSuccess: async () => {
			await queryClient.invalidateQueries({ queryKey: [...queryKeys.professionalOffices, professionalId] });
			navigate("/professional/offices", { replace: true });
		},
		onError: (error) => {
			setCalendarFeedback({
				tone: "error",
	...
	const updateOfficeMutation = useMutation({
		mutationFn: (section: "data" | "rules") => {
			if (!officeForm) throw new Error("No hay datos para guardar.");
			if (section === "data") {
	...
	const toggleDay = (dayOfWeek: number) => {
		setOfficeForm((current) => {
			...
			if (current.scheduleDays.some((day) => day.dayOfWeek === dayOfWeek)) {
				return { ...current, scheduleDays: current.scheduleDays.filter((day) => day.dayOfWeek !== dayOfWeek) };
			}
			return {
				...current,
				scheduleDays: sortScheduleDays([
					...current.scheduleDays,
					{ dayOfWeek, slots: [{ id: buildSlotId(String(dayOfWeek)), startTime: "09:00", endTime: "13:00" }] },
				]),
			};
		});
	};
			...
			return {
				...
				scheduleDays: current.scheduleDays.map((day) =>
					day.dayOfWeek === dayOfWeek
						? { ...day, slots: [...day.slots, { id: buildSlotId(`${dayOfWeek}-${day.slots.length}`), startTime: "15:00", endTime: "19:00" }] }
						: day,
				),
			};
		});
	...
	const updateSlot = (dayOfWeek: number, slotId: string, field: "startTime" | "endTime", value: string) => {
		setOfficeForm((current) => {
			...
			return {
				...
				scheduleDays: current.scheduleDays.map((day) =>
					day.dayOfWeek === dayOfWeek
						? { ...day, slots: day.slots.map((slot) => (slot.id === slotId ? { ...slot, [field]: value } : slot)) }
						: day,
				),
			};
		});
	...
	const removeSlot = (dayOfWeek: number, slotId: string) => {
		setOfficeForm((current) => {
			...
			const nextDays = current.scheduleDays
				.map((day) =>
					day.dayOfWeek === dayOfWeek ? { ...day, slots: day.slots.filter((slot) => slot.id !== slotId) } : day,
				)
	...
	const goToPreviousMonth = () => setDisplayMonth((c) => new Date(c.getFullYear(), c.getMonth() - 1, 1));
	const goToNextMonth = () => setDisplayMonth((c) => new Date(c.getFullYear(), c.getMonth() + 1, 1));
	const goToToday = () => {
		const now = new Date();
	...
	// ── Booked slots for current day (for cancel-day button label) ──
	const bookedForDay = dayItems.flatMap((item) => item.bookedSlots);
				...
				<div className="viewport-section">
					{calendarFeedback ? (
						<div className={`feedback-banner${calendarFeedback.tone === "error" ? " is-error" : " is-success"}`}>
							...
							<Button variant="ghost" className="button-inline" onClick={() => setCalendarFeedback(null)}>Cerrar</Button>
						</div>
					) : null}
				...
				<div className="viewport-section">
					...
					<div className="calendar-layout calendar-layout-viewport">
						...
						<AgendaDayPanel
							...
							mode="special"
							onCancelBookedSlot={(id) => setAppointmentToCancel(id)}
							onBlockFreeSlot={(slot) => { setSlotToBlock(slot); setBlockReason(""); }}
							onUnblockBlockedSlot={(slot) => setBlockToUnblock({ blockId: slot.blockId, timeLabel: slot.time })}
							onDayAction={() => {
								if (hasFullDayBlock) {
									const fullDayBlock = dayBlocks.find((b) => b.type === "full_day");
									if (!fullDayBlock) return;
									setBlockToUnblock({ blockId: fullDayBlock.id, timeLabel: "dia completo", isFullDay: true });
									return;
								}
					...
					{!hasFullDayBlock && bookedForDay.length > 0 ? (
						<div className="form-actions" style={{ marginTop: 8 }}>
							<Button
								variant="ghost"
								onClick={() => { setIsCancellingDay(true); setCancelDayReason(""); }}
								disabled={cancelDayMutation.isPending}
							>
								Cancelar todos los turnos del dia (sin bloquear)
							</Button>
						</div>
					) : null}
				</div>
	...
	return (
		<div className="page-stack viewport-page">
			...
			<ConfirmDialog
				...
				description="Se cancelara este turno del consultorio."
				tone="danger"
				confirmLabel={cancelAppointmentMutation.isPending ? "Cancelando..." : "Cancelar turno"}
				onClose={() => setAppointmentToCancel(null)}
				onConfirm={() => { if (!appointmentToCancel || cancelAppointmentMutation.isPending) return; cancelAppointmentMutation.mutate(appointmentToCancel); }}
			/>
			...
			<ConfirmDialog
				...
				onConfirm={() => { if (!appointmentToComplete || completeAppointmentMutation.isPending) return; completeAppointmentMutation.mutate(appointmentToComplete); }}
			/>

			<ConfirmDialog
				isOpen={Boolean(slotToBlock)}
				title="Bloquear turno"
				description={slotToBlock ? `Se bloqueara el horario de las ${slotToBlock.time} del ${formatNumericDate(selectedDate)}.` : ""}
				tone="danger"
				confirmLabel={blockSlotMutation.isPending ? "Bloqueando..." : "Bloquear turno"}
				onClose={() => { setSlotToBlock(null); setBlockReason(""); }}
				onConfirm={() => { if (blockSlotMutation.isPending || !slotToBlock) return; blockSlotMutation.mutate(); }}
			>
				<Input label="Motivo del bloqueo" value={blockReason} onChange={(e) => setBlockReason(e.target.value)} placeholder="Ej. Reunion, ausencia" />
			</ConfirmDialog>
			...
			<ConfirmDialog
				...
				onClose={() => { setIsBlockingDay(false); setBlockReason(""); }}
				onConfirm={() => { if (blockDayMutation.isPending) return; blockDayMutation.mutate(); }}
			>
				<Input label="Motivo del bloqueo" value={blockReason} onChange={(e) => setBlockReason(e.target.value)} placeholder="Ej. Congreso, licencia" />
			</ConfirmDialog>

			<ConfirmDialog
				isOpen={isCancellingDay}
				title="Cancelar turnos del dia"
				description={`Se cancelaran todos los turnos activos del ${formatNumericDate(selectedDate)} sin bloquear el dia. Los horarios seguiran disponibles para nuevas reservas.`}
				tone="danger"
				confirmLabel={cancelDayMutation.isPending ? "Cancelando..." : "Cancelar turnos"}
				onClose={() => { setIsCancellingDay(false); setCancelDayReason(""); }}
				onConfirm={() => { if (cancelDayMutation.isPending) return; cancelDayMutation.mutate(); }}
			>
				<Input label="Motivo de cancelacion" value={cancelDayReason} onChange={(e) => setCancelDayReason(e.target.value)} placeholder="Ej. Emergencia personal" />
			</ConfirmDialog>
			...
			<ConfirmDialog
				...
				description={
					blockToUnblock?.isFullDay
						? `El ${formatNumericDate(selectedDate)} volvera a quedar disponible.`
						: blockToUnblock
							? `El horario de las ${blockToUnblock.timeLabel} volvera a quedar disponible.`
							: ""
				}
				confirmLabel={unblockBlockMutation.isPending ? "Desbloqueando..." : "Desbloquear"}
				onClose={() => setBlockToUnblock(null)}
				onConfirm={() => { if (!blockToUnblock || unblockBlockMutation.isPending) return; unblockBlockMutation.mutate(blockToUnblock.blockId); }}
			/>

			<ConfirmDialog
				isOpen={isDeleteOfficeOpen}
				title="Eliminar consultorio"
				description={`Se eliminara "${officeQuery.data?.name}" con todos sus horarios, turnos y bloqueos asociados. Esta accion no se puede deshacer.`}
				tone="danger"
				confirmLabel={deleteOfficeMutation.isPending ? "Eliminando..." : "Eliminar consultorio"}
				onClose={() => setIsDeleteOfficeOpen(false)}
				onConfirm={() => { if (deleteOfficeMutation.isPending) return; deleteOfficeMutation.mutate(); }}
			/>
		</div>
	);
}